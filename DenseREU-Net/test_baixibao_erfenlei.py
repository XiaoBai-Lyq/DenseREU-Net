...
img_dir = 'baixobao_2fnelei/test/img'  #原始图片npy
label_dir = 'baixobao_2fnelei/test/label'  #原始标签npy
...
if config['name'] == 'UNET':
    ...
    net = UNet_2Plus.Unet2_Plus(num_classes=1)
    ...
    net = Se_PPP_ResUNet(3, 1, deep_supervision=False)
elif config['name'] == 'TransUNet':
    config_vit = CONFIGS_ViT_seg['R50-ViT-B_16']
    net = ViT_seg(config_vit, img_size=256, num_classes=1)
elif config['name'] == 'attention_unet':
    net = attention_unet.AttU_Net(3, 1)
elif config['name'] == "resunet_plus":
    net = res_unet_plus.ResUnetPlusPlus(3, 1)
    ...
    net = UNet_dense5.Unet(3,1)
elif config['name'] == "UNet_dense6":
    net = UNet_dense6.Unet(3,1)
else:
    raise ValueError("Wrong Parameters")

net.load_state_dict(torch.load(('E:\\Medical_imageseg_yes_baixibao\\checpoint\\baixibao_erfenlei\\{}_base\\bestmodel_baixibao_0.0001_final.pth').format(config["name"])))
net.cuda()
...
if __name__ == "__main__":
    ...
    for i in range(len(predimg)):
        ...
        if i < 10:
            ...
            imgx = add_alpha_channel(clip_image_path2, 1)
            imgx2 = add_alpha_channel(clip_image_path2, 1)

            clip_image_path = os.path.join(labelnpytoimg_dir, test_pre_name)
            io.imsave(clip_image_path, label)
            labelx = add_alpha_channel(clip_image_path,1)
            ...
            prelabelx = add_alpha_channel(clip_label_path, 1)
        ...
        if i>=100 and i < 1000:
            ...
            clip_image_path2 = os.path.join(imgnpytoimg_dir, test_pre_name)
            io.imsave(clip_image_path2, x)
            imgx = add_alpha_channel(clip_image_path2, 0.90)
            imgx2 = add_alpha_channel(clip_image_path2, 0.90)

            clip_image_path = os.path.join(labelnpytoimg_dir, test_pre_name)
            io.imsave(clip_image_path, label)
            labelx = add_alpha_channel(clip_image_path, 0.90)

            image_together(imgx, labelx, imgandlabel, test_pre_name)

            clip_label_path = os.path.join(forecast_label, test_pre_name)
            io.imsave(clip_label_path, preimg2)
            prelabelx = add_alpha_channel(clip_label_path, 0.90)
            image_together(imgx2, prelabelx, imgandlabel2, test_pre_name)
