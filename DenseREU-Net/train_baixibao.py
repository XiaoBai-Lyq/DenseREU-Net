...
from metrics import iou_score,dice_score,f1_scorex
...
def main():
    ...
    os.makedirs('checpoint/baixibao/{}'.format(file_name), exist_ok=True)
    ...
    print('-' * 20)
    #save configuration
    with open('checpoint/baixibao/{}/config.yml'.format(file_name), 'w') as f:
        yaml.dump(config, f)
    ...
    criterion = nn.CrossEntropyLoss(weight=None, reduction="mean").cuda()  # 初始化损失函数
    # cudnn.benchmark = True
    """
    create model
    """
    ...
    if config['name'] == "UNET":
        model = UNett_batcnnorm.Unet(3, 7)
        ...
        model = UNet_2Plus.Unet2_Plus(num_classes=7)
    elif config['name'] == 'SAR_UNet':
        model = Se_PPP_ResUNet(3, 7, deep_supervision=False)
    elif config['name'] == 'TransUNet':
        config_vit = CONFIGS_ViT_seg['R50-ViT-B_16']
        model = ViT_seg(config_vit, img_size=256, num_classes=7)
    elif config['name'] == 'attention_unet':
        model = attention_unet.AttU_Net(3, 7)
    elif config['name'] == "resunet_plus":
        model = res_unet_plus.ResUnetPlusPlus(3, 7)
        ...
        model = UNet_dense5.Unet(3,7)
    elif config['name'] == "UNet_dense6":
        model = UNet_dense6.Unet(3,7)
    else:
...
###############加载数据路径
    train_img = 'baixibao/train/img'
    train_label = 'baixibao/train/label'
    val_img = 'baixibao/val/img'
    val_label = 'baixibao/val/label'
    ...
    for epoch in range(config['epochs']):
        ...
        log.to_csv('checpoint/baixibao/{}/log.csv'.format(file_name), index=False)

        trigger += 1

        if val_log['dice'] > best_dice:
            torch.save(model.state_dict(), 'checpoint/baixibao/{}/bestmodel_baixibao_{}_final.pth'.format(file_name,config["lr"]))
